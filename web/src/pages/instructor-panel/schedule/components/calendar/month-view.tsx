...
export function MonthView({ currentDate }: MonthViewProps) {
  ...
  const getClassesForDay = (date: Date) => {
    ...
    return mockClasses.sort(() => Math.random() - 0.5).slice(0, Math.floor(Math.random() * 4) + 2)
  }
    ...
    <>
      <div className="grid grid-cols-7 gap-px bg-muted">
        ...
        {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(day => (
          <div key={day} className="h-10 bg-background p-2 text-center text-sm font-medium">
            {day}
          </div>
        ))}
              ...
              <div className="flex justify-between items-center mb-1">
                <span
                  className={`text-sm inline-flex items-center justify-center
                      ${isCurrentDay ? 'font-bold text-lg text-blue-600 dark:text-blue-400' : ''}`}
                >
                  {format(day, 'd')}
                </span>
        ...
        <DrawerContent>
          <div className="mx-auto w-full max-w-4xl">
            <DrawerHeader>
              ...
              <DrawerDescription>{selectedDayClasses.length} classes scheduled</DrawerDescription>
            </DrawerHeader>
