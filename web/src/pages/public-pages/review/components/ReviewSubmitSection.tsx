...
export function ReviewSubmitSection({ completedSteps, onSubmit }: ReviewSubmitSectionProps) {
  return (
    <Card className="p-8 bg-muted/30">
      <div className="text-center space-y-4">
        <div className="flex justify-center mb-4">
          <CheckCircle2
            className={`w-12 h-12 ${completedSteps.size > 0 ? 'text-primary' : 'text-muted'}`}
          />
        </div>
        ...
        >
          Submit {completedSteps.size > 0 ? `${completedSteps.size} ` : ''}Review
          {completedSteps.size !== 1 ? 's' : ''}
        </Button>
      </div>
    </Card>
  )
