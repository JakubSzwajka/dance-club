        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-7 gap-4">
            {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(day => (
              <div key={day} className="text-center font-medium text-sm p-2 bg-muted rounded-t-md">
                {day}
              </div>
            ))}
            ...
            {['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'].map(
              day => (
                <div key={day} className="min-h-[100px] p-2 border rounded-md bg-card">
                  {schedule
                    .filter(item => item.day === day)
                    .map(item => (
                      <div key={item.id} className="text-xs p-1 mb-1 bg-primary/10 rounded">
