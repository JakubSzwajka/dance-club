...
class EventsSearchEngineService:
    def get_events_with_filters(self, location_id: Optional[str] = None, instructor_id: Optional[str] = None, start_date: Optional[str] = None, end_date: Optional[str] = None, limit: int = 10) -> List[SpecialEventSchema]:
        ...
        if instructor_id:
            events = events.filter(instructor_id=instructor_id)

        if start_date:
            events = events.filter(datetime__gte=start_date)

        if end_date:
            events = events.filter(datetime__lte=end_date)

        events = events[:limit]

        return [event.to_schema() for event in events]

    def get_events_near_location(self, latitude: Optional[float] = None, longitude: Optional[float] = None, start_date: Optional[str] = None, end_date: Optional[str] = None, limit: int = 10) -> List[SpecialEventSchema]:
        if latitude is None or longitude is None:
            return [event.to_schema() for event in SpecialEvent.objects.all()[:limit]]
