...
from reviews.constants import (
    TEACHING_STYLE_MIN, TEACHING_STYLE_MAX,
    FEEDBACK_APPROACH_MIN, FEEDBACK_APPROACH_MAX,
    PACE_OF_TEACHING_MIN, PACE_OF_TEACHING_MAX,
    BREAKDOWN_QUALITY_MIN, BREAKDOWN_QUALITY_MAX,
    RATING_MIN, RATING_MAX,
    TemperatureType, TEMPERATURE_OPTIONS,
    VOLUME_MIN, VOLUME_MAX,
    MUSIC_STYLE_MIN, MUSIC_STYLE_MAX,
    WaitingAreaType, WAITING_AREA_OPTIONS,
)

class TeachingApproachSchema(BaseModel):
    teaching_style: Annotated[int, Field(
        ge=TEACHING_STYLE_MIN,
        le=TEACHING_STYLE_MAX,
        description="Balance between structured (0) and casual (100) teaching"
    )]
    feedback_approach: Annotated[int, Field(
        ge=FEEDBACK_APPROACH_MIN,
        le=FEEDBACK_APPROACH_MAX,
        description="Balance between verbal (0) and hands-on (100) feedback"
    )]
    pace_of_teaching: Annotated[int, Field(
        ge=PACE_OF_TEACHING_MIN,
        le=PACE_OF_TEACHING_MAX,
        description="Balance between methodical (0) and fast-paced (100)"
    )]
    breakdown_quality: Annotated[int, Field(
...
class EnvironmentSchema(BaseModel):
    floor_quality: Annotated[int, Field(
        ge=RATING_MIN,
        le=RATING_MAX,
        description="Rating of floor quality (1-5)"
    )]
    crowdedness: Annotated[int, Field(
        ge=RATING_MIN,
        le=RATING_MAX,
        description="Rating of space comfort/crowdedness (1-5)"
    )]
    ventilation: Annotated[int, Field(
        ge=RATING_MIN,
        le=RATING_MAX,
    ...
    def validate_temperature(cls, v: str) -> str:
        if v not in TEMPERATURE_OPTIONS:
            raise ValueError(f"Temperature must be one of: {', '.join(TEMPERATURE_OPTIONS)}")
        return v
...
class MusicSchema(BaseModel):
    volume_level: Annotated[int, Field(
        ge=VOLUME_MIN,
        le=VOLUME_MAX,
        description="Rating of music volume (1-5)"
    )]
    genres: Annotated[List[str], Field(
        min_length=1,
        description="List of music genres played in class"
    )]
    style: Annotated[int, Field(
        ge=MUSIC_STYLE_MIN,
        le=MUSIC_STYLE_MAX,
...
class ChangingRoomSchema(BaseModel):
    available: Annotated[bool, Field(
        description="Whether changing room is available"
    )]
    quality: Annotated[Optional[int], Field(
        None,
        ge=RATING_MIN,
        le=RATING_MAX,
    ...
    notes: Annotated[Optional[str], Field(
        None,
        max_length=500,
        description="Additional notes about the changing room"
    )]
...
class WaitingAreaSchema(BaseModel):
    available: Annotated[bool, Field(
        description="Whether waiting area is available"
    )]
    type: Annotated[Optional[str], Field(
        None,
        description="Type of waiting area"
    )]
    seating: Annotated[Optional[bool], Field(
        None,
        description="Whether seating is available"
    )]
    notes: Annotated[Optional[str], Field(
        None,
        max_length=500,
        description="Additional notes about the waiting area"
    )]
...
class FacilitiesSchema(BaseModel):
    ...
    accepted_cards: Annotated[List[str], Field(
        min_length=1,
        description="List of accepted sports cards"
    )]
